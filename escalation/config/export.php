...
$pending = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Pending' "; // select pending fro mes_srs 
$closed = "SELECT status FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Closed' "; // slect cloased 
$reject = "SELECT status FROM es_srs where date >= '$st_date' AND date<= '$end_date' AND status = 'Rejected' "; //select reject
...
if($pending && $closed){
	...
	$sql = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status in ('Pending','Closed') "; // select pending & closed from mes_srs 
	$pending = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Pending' "; // select pending fro mes_srs 
	$closed = "SELECT status FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Closed' "; // slect cloased 
...
*/
	...
	$_SESSION['ex'] = 'pr';
		...
		$sql = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status in ('Pending','Rejected') "; // select pending & closed from mes_srs 
		$pending = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Pending' "; // select pending fro mes_srs 
		$reject = "SELECT status FROM es_srs where date >= '$st_date' AND date<= '$end_date' AND status = 'Rejected' "; //select reject
	...
	' <?php echo "All : " 		. $result->num_rows ."<br>";  ?>';	// to print num_rows of reject
		...
		$_SESSION['ex'] = 'cr';
			...
			$sql = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status in ('Closed','Rejected') "; // select pending & closed from mes_srs
			$reject = "SELECT status FROM es_srs where date >= '$st_date' AND date<= '$end_date' AND status = 'Rejected' "; //select reject
			$closed = "SELECT status FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Closed' "; // slect cloased 
...
document.getElementById("right").innerHTML=
	...
	$closed = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Closed' "; // slect cloased 
		$result_closed = $connt->query($closed); 
		?>
		<script>
document.getElementById("right").innerHTML=
	...
	<?php	
		...
		if($result_closed->num_rows > 0){
			...
			$_SESSION['ex'] = 'r';
				...
				$reject = "SELECT * FROM es_srs where date >= '$st_date' AND date<= '$end_date' AND status = 'Rejected' "; //select reject
				$result_reject = $connt->query($reject);  
			?>	
				<script>
document.getElementById("right").innerHTML=
	...
	<?php			
		...
		$sql = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' And status != 'Suspend'"; // slect cloased 
		$pending = "SELECT * FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Pending' "; // select pending fro mes_srs 
		$closed = "SELECT status FROM es_srs where date >= '$st_date' AND date <= '$end_date' AND status = 'Closed' "; // slect cloased 
		$reject = "SELECT status FROM es_srs where date >= '$st_date' AND date<= '$end_date' AND status = 'Rejected' "; //select reject
